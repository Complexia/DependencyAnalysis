...
//		}
		 ...
		 */
		ArrayList<SimpleService> remComServList = new ArrayList<SimpleService>();/*list to store the services generated by remote computation, these won't be fully fledged
																			   *services as they're just going to be using the name and children fields to denote roughly defined groupings.
																			   */
		ArrayList<SimpleService> visited = new ArrayList<SimpleService>(); /* keep a list of services that have been looked at to avoid doubling up*/
		for(SimpleService i : services)
		{
			if(!visited.contains(i))
			{
				SimpleService remComServ = new SimpleService();
				remComServ.setName("RS" + remComServList.size());
				remComServList.add(remComServ);
				recursiveRemComFind(i,visited,AL, remComServ);
			}
		}
	...
	private void recursiveRemComFind(SimpleService target, ArrayList<SimpleService> visited, ArrayList<IOVariable> chosenEdges, SimpleService remComServ)
	{
		visited.add(target);
		//implement the solution here
		for(IOVariable i : target.getOutputService())
		{
			if(chosenEdges.contains(i))
			{
				chosenEdges.remove(i);//prevents it from being re explored which would cause infinite loops and stack overflows in many cases
				//let the "temporary workaround"ing commence.
				//also this is going to be super inefficient and slow but whatever as long as it works we good
				for(SimpleService inServ : Factory.getElementaryServices())
				{
					if(inServ.getInputService().contains(i)) recursiveRemComFind(inServ, visited, chosenEdges, remComServ);
				}
			}
		}
		
		for(IOVariable i : target.getInputService())
		{
			if(chosenEdges.contains(i))
			{
				chosenEdges.remove(i);//prevents it from being re explored which would cause infinite loops and stack overflows in many cases
				//let the "temporary workaround"ing commence.
				//also this is going to be super inefficient and slow but whatever as long as it works we good
				for(SimpleService outServ : Factory.getElementaryServices())
				{
					if(outServ.getOutputService().contains(i)) recursiveRemComFind(outServ, visited, chosenEdges, remComServ);
				}
			}
		}
	}
