...
//		});
	 ...
	 * starting at an initial service then recurse over connections staring with outputs then moving onto inputs, keep a list of visited nodes
	...
	private void RemoteComputationHP(ArrayList<IOVariable> AL) 
	{
		...
		columnNames = new Object[]{"High performance component", "Component services"};
		final int HIGH_PERF = 0;
		final int COMP_SERV = 1;
		tableData = new Object[services.size()][columnNames.length];
		int index = 0;
		for(SimpleService highPerfServ : remComServList)
		{
			tableData[index][HIGH_PERF] = highPerfServ.getName();
			for(SimpleService kid : highPerfServ.getChildren())
			{
				tableData[index][COMP_SERV] = kid.getName();
				index++;
			}
			index++;
		}
	...
	private void recursiveRemComFind(SimpleService target, ArrayList<SimpleService> visited, ArrayList<IOVariable> chosenEdges, SimpleService remComServ)
	{
		...
		for(IOVariable i : target.getOutputService())
		{
			if(chosenEdges.contains(i))
			{
				...
				{
					if(inServ.getInputService().contains(i)) remComServ.addChild(inServ); recursiveRemComFind(inServ, visited, chosenEdges, remComServ);
				}
			}
		}
		...
		for(IOVariable i : target.getInputService())
		{
			if(chosenEdges.contains(i))
			{
				...
				{
					if(outServ.getOutputService().contains(i)) remComServ.addChild(outServ); recursiveRemComFind(outServ, visited, chosenEdges, remComServ);
				}
			}
		}
	}
...
//	 * @deprecated
//	 * @param AL1
//	 * @return the strongly connected service
//	 */
