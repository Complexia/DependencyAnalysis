...
//		}
		 ...
		 */
		ArrayList<SimpleService> remComServs = new ArrayList<SimpleService>();/*list to store the services generated by remote computation, these won't be fully fledged
		...
		for(SimpleService i : services)
		{
			...
			{
				recursiveRemComFind(i,visited,AL, remComServs);
			}
		}
	...
	private void recursiveRemComFind(SimpleService target, ArrayList<SimpleService> visited, ArrayList<IOVariable> chosenEdges, ArrayList<SimpleService> remComServs)
	{
		visited.add(target);
		...
		for(IOVariable i : target.getOutputService())
		{
			if(chosenEdges.contains(i))
			{
				...
				{
					if(inServ.getInputService().contains(i)) recursiveRemComFind(inServ, visited, chosenEdges, remComServs);
				}
			}
		}
	}
